...
for c in $TOMLS; do
  ...
  $c (exit $RC): $(sed -n '1,2p' "$OUT" | tr '\n' ' ' | cut -c1-140)"
    # Preserve package diagnostics for audit and recovery; CI runners are ephemeral.
    continue
  fi
...
# Preserve aggregate diagnostics for audit and recovery; CI runners are ephemeral.
exit 0
